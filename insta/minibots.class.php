...
Class Minibots 
{
	...
	public function readFacebookPageCounters($url) {
		...
		foreach($matches[0] as $m) {
			...
			if($name=="description") {
				...
				preg_match_all("/ ([0-9,]*) /",$content,$counters);
				return array(
					"likes" => preg_replace("/[^0-9]/","",$counters[0][0]),
					"talking" => preg_replace("/[^0-9]/","",$counters[0][1]),
					"here" => preg_replace("/[^0-9]/","",$counters[0][2]),
				);
			}
		}
		...
		if(isset($b->entry_data->ProfilePage[0]->user) && isset($b->entry_data->ProfilePage[0]->user->media)) {
			...
			if(isset($b->entry_data->ProfilePage[0]->user->media->nodes)) {
				foreach($b->entry_data->ProfilePage[0]->user->media->nodes as $pic) {
					$thumb = str_replace("/s640x640/","/s150x150/",$pic->thumbnail_src);
					$low = str_replace("/s640x640/","/s320x320/",$pic->thumbnail_src);
					$a[] = array(
						...
						"full_resolution"=>$pic->display_src,
						"thumbnail"=>$thumb,
	...
	*/
	public function getInstagramPicsByTag($tag) {
		...
		if(isset($b->entry_data->TagPage[0]) && isset($b->entry_data->TagPage[0]->tag->media)) {
			$a = array(); // all
			$bb = array(); // top posts
			$cc = array(); // ultime
			if(isset($b->entry_data->TagPage[0]->tag->media->nodes)) {
				foreach($b->entry_data->TagPage[0]->tag->top_posts->nodes as $pic) {
					$thumb = str_replace("/s640x640/","/s150x150/",$pic->thumbnail_src);
					$low = str_replace("/s640x640/","/s320x320/",$pic->thumbnail_src);
					$a[] = array(
						"link"=>"https://www.instagram.com/p/".$pic->code."/", 
				...
				foreach($b->entry_data->TagPage[0]->tag->media->nodes as $pic) {
					$found = false;
					for($i=0;$i<count($a);$i++) {
						if($a[$i]["code"] == $pic->code) {
							$found = true;
							break;
						}
					}
			...
			return array("user"=>null,"pics"=>$a,"top"=>$bb,"last"=>$cc);
		}
		return false;
	}
